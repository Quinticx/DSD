...
if ~isfield(gaDat,'MAXGEN')
    gaDat.MAXGEN = 30; %gaDat.NVAR*30+10;
end
...
%==========================
%=== yadong new try  (random generated)
%==========================
...
% ublb=ones(Nind,1)*m;
% lb=ones(Nind,1)*FieldDR(1,:);
% chrom_tem=ublb.*aux+lb;
% 
% aaa = chrom_tem(:,1)/1000;
% aaaa = round(aaa,1);
% aa = aaaa*1000;
% 
% b = chrom_tem(:,2);
...
%==========================
%=== yadong new try 2 (uniformally generated)
%==========================

a_top = max(FieldDR(:,1));
a_bottom = min(FieldDR(:,1));
aa = a_bottom+(a_top-a_bottom)/Nind:(a_top-a_bottom)/Nind:a_top;

b_top = max(FieldDR(:,2));
b_bottom = min(FieldDR(:,2));
bb = b_bottom+(b_top-b_bottom)/Nind:(b_top-b_bottom)/Nind:b_top;

c_top = max(FieldDR(:,3));
c_bottom = min(FieldDR(:,3));
cc = c_bottom+(c_top-c_bottom)/Nind:(c_top-c_bottom)/Nind:c_top;
...
auxf1=aux*FieldDR(1,:);
auxf2=aux*FieldDR(2,:);
NewChrom_tem = (NewChrom>auxf2).*auxf2+(NewChrom<auxf1).*auxf1+(NewChrom<=auxf2 & NewChrom>=auxf1).*NewChrom;

aaa = NewChrom_tem(:,1)/1000;
aaaa = round(aaa,1);
aa = aaaa*1000;

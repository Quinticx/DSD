...
%         save("ka_forward.mat", "Kw_Ka_F", "fa_Ka_F", "fb_Ka_F");
    end
    loaded_Ku_F = 1;
end
...
% mean_fa_Ka_F = sum(NG.*fa_Ka_F );
% mean_fb_Ka_F = sum(NG.*0.1.*fb_Ka_F);
...
% mean_ff_Ka = sum(NG.*real(fa_Ka_F-fb_Ka_F));
% 
...
% Zh_Ka_F(i) = sum(NG.*0.1.*( abs_fa_square_Ka.*AA + abs_fb_square_Ka.*BB + 2*CC.*abs_fa_Ka.*abs_fb_Ka))*4*(lamuda_Ka*10)^4/pi^4/(abs(Kw_Ka))^2;
% 
